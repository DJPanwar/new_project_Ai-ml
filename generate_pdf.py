...
import pandas as pd
import json
from checksum import add_checksum, get_signature
import numpy as np
import cv2
...
class Generator:
    ...
    def create_pdf(self, data, embed_file, certificate_name, doc_type, graph_img, attach_data=True, attach_graph=False, version=1):
        ...
        ##--------------------new-start------------------------------------------------------------------------------
        # Normalize the embed_file path to use forward slashes for LaTeX
        latex_embed_file_path = embed_file.replace(os.sep, '/')
        embedding = f"\\embedfile{{{latex_embed_file_path}}}" if attach_data else ""
        ##--------------------new-end------------------------------------------------------------------------------
        latex_template = f"""
        \\DocumentMetadata{{
        pdfversion=1.7,
...
##--------------------old-start------------------------------------------------------------------------------
        ...
        # for ext in [".aux", ".log", ".out", ".toc", ".blg", ".bbl", ".tex"]:
...
##--------------------new-start------------------------------------------------------------------------------
        # Save the LaTeX code to a file
        aux_file = f"{certificate_name}.tex" # This creates the .tex file next to app.py
        tex_file = f"./{aux_file}"
        pdf_dir = f"./static/pdfs/{certificate_name}" # This is the folder path
        pdf_file = f"{pdf_dir}/{version}.pdf" # THIS IS THE CRUCIAL CHANGE: Use version for PDF filename

        # Create the directory for the specific certificate (e.g., static/pdfs/N23070405_D3.03_C-037)
        if not os.path.exists(pdf_dir):
            os.makedirs(pdf_dir)
            print(f"Created PDF directory: {pdf_dir}") # Added print for clarity
        ...
        # convert tex to pdf
        try:
            # subprocess.run(["lualatex", "-output-directory", pdf_dir, tex_file], capture_output=True)
            # subprocess.run(["lualatex", "-output-directory", pdf_dir, tex_file], capture_output=True)
            # Using -output-directory makes it put .aux, .log etc. directly into pdf_dir
            # However, your existing code moves the .pdf file only.
            # Let's keep your current approach of generating in root and moving just the PDF,
            # then clean up temp files from root.
            subprocess.run(["lualatex", tex_file], capture_output=True, cwd=".") # Run in current directory
            subprocess.run(["lualatex", tex_file], capture_output=True, cwd=".")
            print("PDF created successfully.")
        except subprocess.CalledProcessError as e:
            print("An error occurred while compiling the LaTeX document:")
            print(e.stderr.decode('utf-8')) # Decode for better readability
            raise # Re-raise to signal failure to app.py

        # Move the generated PDF file to the pdfs folder
        # Ensure the correct path for source and destination
        source_pdf_file_path = aux_file.replace(".tex", ".pdf")
        if os.path.exists(source_pdf_file_path):
            os.replace(source_pdf_file_path, pdf_file)
            print(f"Moved '{source_pdf_file_path}' to '{pdf_file}'")
        else:
            print(f"Error: Compiled PDF '{source_pdf_file_path}' not found after LaTeX compilation.")
            raise FileNotFoundError(f"Compiled PDF '{source_pdf_file_path}' not found.")


        # Generate Checksum (now for the specific versioned file)
        # Note: The original generate_checksum takes file_path.
        # This will be used by app.py to update the *main* checksums table.
        # No need to call add_checksum here, app.py will do it.
        # expected_checksum = self.generate_checksum(pdf_file) # This line moved to app.py
        # print(f"Generated checksum for {data['certificate_no']}: {expected_checksum}") # This line moved to app.py

        # Store Checksum - This is now handled by app.py's call to checksum.insert_pdf_record and checksum.add_checksum
        # response = add_checksum({ ... }) # REMOVE THIS CALL FROM HERE
        # print(response) # REMOVE THIS PRINT FROM HERE

        #remove extra files generated by LaTeX from the root directory
        for ext in [".aux", ".log", ".out", ".toc", ".blg", ".bbl", ".tex"]:
            temp_file = aux_file.replace(".tex", ext)
            if os.path.exists(temp_file):
                os.remove(temp_file)
                print(f"Cleaned up temporary file: {temp_file}")
            else:
                print(f"Temporary file not found for cleanup: {temp_file}")
##--------------------new-end------------------------------------------------------------------------------
    ...
    def execute_pdf_generator(self, excel_file, doc_type):
        ...
        return file_name, data_template, data_to_send