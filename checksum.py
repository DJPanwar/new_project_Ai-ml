...
def get_recent_pdf():
    ...
    session.close()
    if result is None:
        print("get_recent_pdf: No PDF record found.")
        return None  # Or return a default path/value if needed.
...
##--------------------new-start------------------------------------------------------------------------------
def pdf_exists(pdf_directory):
    # This function should only check if a *record for the directory* exists, not necessarily the file.
    # The versioning logic will handle the file existence and incrementing.
    session = Session()
    try:
        # Check if *any* record exists for this pdf_directory
        exists = session.query(DccPdf).filter_by(pdf_directory=pdf_directory).first() is not None
        print(f"pdf_exists: DB record for directory '{pdf_directory}' found: {exists}")
        return exists
    except Exception as e:
        print(f"Error checking PDF directory existence in DB: {e}")
        return False
    finally:
        session.close()

# NEW FUNCTION to get the next version number
# Ensure func is imported at the top

def get_next_pdf_version(pdf_directory):
    session = Session()
    try:
        # Find the maximum version for the given pdf_directory
        max_version = session.query(func.max(DccPdf.version)).filter(
            DccPdf.pdf_directory == pdf_directory
        ).scalar()

        if max_version is None:
            # No existing versions for this directory, start with 1
            return 1
        else:
            # Increment the highest existing version
            return max_version + 1
    except Exception as e:
        print(f"Error getting next PDF version for {pdf_directory}: {e}")
        # Default to version 1 in case of error, to allow creation
        return 1
    finally:
        session.close()
##--------------------new-end------------------------------------------------------------------------------